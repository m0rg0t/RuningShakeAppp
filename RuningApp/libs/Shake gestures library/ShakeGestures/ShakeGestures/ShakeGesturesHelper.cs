...
namespace ShakeGestures
{
    public class ShakeGesturesHelper
    {
        ...
        private bool ProcessShakeSignal()
        {
            ...
            ShakeTypeExtended shakeTypeEx = new ShakeTypeExtended() ;
            ...
            if ((xCount >= yCount) & (xCount >= zCount) & (xCount >= MinimumShakeVectorsNeededForShake))
            {
                ...
                shakeTypeEx.ShakeTypeStr = "X";
                shakeTypeEx.MaxValues = CountMaxValues(shakeType.Value);
            }
            ...
            if (shakeType != null)
            {
                ...
                {
                    localShakeGesture(this, new ShakeGestureEventArgs(shakeTypeEx));
                }
                return true;
            }
        ...
        private MaxVal CountMaxValues(ShakeType shakeType)
        {
            //int countSignsChanges = 0;
            //int currentSign = 0;
            MaxVal _maxvalues = new MaxVal();
            ...
            for (int i = 0; i < _shakeSignal.Count; ++i)
            {
                ...
                {
                    case ShakeType.X:
                        if (_maxvalues.maxx < _shakeSignal[i].X)
                        {
                            _maxvalues.maxx = _shakeSignal[i].X;
                        };

                        if (_maxvalues.minx > _shakeSignal[i].X)
                        {
                            _maxvalues.minx = _shakeSignal[i].X;
                        };
                        //currentSign = Math.Sign(_shakeSignal[i].X);
                        break;

                    case ShakeType.Y:
                        if (_maxvalues.miny > _shakeSignal[i].Y)
                        {
                            _maxvalues.miny = _shakeSignal[i].Y;
                        };
                        //currentSign = Math.Sign(_shakeSignal[i].Y);
                        break;

                    case ShakeType.Z:
                        if (_maxvalues.minz < _shakeSignal[i].Z)
                        {
                            _maxvalues.minz = _shakeSignal[i].Z;
                        };
